...
// ================= DELETE ALL GRADE 6 =================
const deleteAllGrade6 = async () => {
  if (stage !== "grade6") return; // يظهر فقط للصف السادس
  if (!window.confirm("⚠️ هل أنت متأكد من حذف كل الأطفال في سنة سادسة؟")) return;
  ...
  try {
    ...
    setRows([]); // تفريغ الـ state
    cachedRows.current = [];
    alert("✅ تم حذف جميع الأطفال في سنة سادسة");
  } catch (error) {
    console.error("خطأ في حذف جميع الأطفال:", error);
    ...
    <div className="min-h-screen p-6">
      <div className="bg-white/90 p-6 rounded-2xl shadow-xl">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {/* زر حذف كل الأطفال للصف السادس فقط */}
          {stage === "grade6" && (
            <button
              onClick={deleteAllGrade6}
              className="px-4 py-2 bg-red-600 text-white rounded-xl hover:bg-red-700 transition"
             >
              ❌ حذف كل الأطفال في سنة سادسة
             </button>
            )}
